...
interface MusicStore {
  ...
  updateSong: (
    id: string,
    data: Partial<Pick<Song, "title" | "releaseYear">>,
  ) => Promise<void>;
  updateAlbum: (
    id: string,
    data: Partial<Pick<Album, "title" | "releaseYear">>,
  ) => Promise<void>;
}
...
export const useMusicStore = create<MusicStore>((set) => ({
  ...
  updateAlbum: async (id, data) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axiosInstance.patch(`/admin/albums/${id}`, data);
      
      set((state) => ({
        albums: state.albums.map((a) => (a._id === id ? response.data : a)),
      }));
  ...
  updateSong: async (id, data) => {
    ...
    try {
      ...
      toast.success("Song update succesfully");
    } catch (error: any) {
